...
def format_context(documents: list[Document]) -> str:
    ...
    for doc in documents:
        print(doc.metadata)
        if 'title' in doc.metadata['data']:
            paper_title = doc.metadata['data']['title']
            formated_context += f'\n"{doc.page_content}" from paper: {paper_title}\n'
        else:
            formated_context += f'\n{doc.page_content}\n'
...
def create_conversation(chat_history: list) -> list[list]:
    try:
        ...
        system_prompt = '''You are a reesarch assistant that always cites their sources when answering questions about research papers.'''
